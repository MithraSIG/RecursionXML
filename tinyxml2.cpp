...
XMLError XMLDocument::SaveFileWithMacro(const char* filename)
{
    if (!filename) {
        TIXMLASSERT(false);
        SetError(XML_ERROR_FILE_COULD_NOT_BE_OPENED, 0, "filename=<null>");
    ...
    SaveFile(fp, false, true);
    fclose(fp);
    return _errorID;
}

XMLError XMLDocument::SaveFile( FILE* fp, bool compact, bool with_macro )
{
    // Clear any error from the last save, otherwise it will get reported
    // for *this* call.
    ClearError();
    XMLPrinter stream(fp, compact, 0, with_macro);
    Print( &stream );
    return _errorID;
}
...
void XMLDocument::PrintErrorStr()
{
    printf("%s\n", ErrorStr());
}
...
XMLPrinter::XMLPrinter( FILE* file, bool compact, int depth,bool with_macro) :
    ...
    _buffer()
{
    ...
    if (!with_macro)_restrictedEntityFlag[static_cast<unsigned char>('&')] = true;
